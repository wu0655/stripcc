...
/**************************************************************************
*
*  FUNCTION: add_warning_cc_to_files()
*
*  PARAMETERS: 
*
*  RETURNS: The number of files which are added "warning"
*
*  DESCRIPTION:  Search the direcotry and find the match files. such *.h,*.c
*
***************************************************************************/
static int
...
*
*  DESCRIPTION:  
*	1. fork a child thread to build 
*	2. read the err information by pipe and parse them and add the warning information to a rets list.
*
***************************************************************************/
    ...
    while (1) {
        ...
        while (fgets(linebuf, sizeof(linebuf), fp) != NULL) {
            ...
            if (count % 1200 == 0) {
                printf("\b\b\b   \b\b\b");
                fflush(stdout);
            } else if (count % 300 == 0) //if count==300, print one line. good.
            {
                printf(".");
                fflush(stdout);
            }
